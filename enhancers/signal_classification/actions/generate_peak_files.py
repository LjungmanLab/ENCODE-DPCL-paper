...
def create_final_df(bipeaks, unipeaks):
    ...
    allpeaks = pd.concat([bp, up])
    allpeaks = allpeaks.sort_values(by = ['chr', 'start'])
    return allpeaks
...
def main(bed_filepath, dist):
    ...
    unipeaks0 = unique_unipeaks(filtered_df, bipeaks0, dedup_cols) #generate_ids not working? No seems to be... must be merge that is wrong?
    
    print("\nGenerating merged all_peak file")
    all_peaks0 = create_final_df(bipeaks0, unipeaks0)

    #Output files
    bipeaks = bipeaks0.rename(columns={'bipeak_chr': '#bipeak_chr'})
    unipeaks = unipeaks0.rename(columns={'unipeak_chr': '#unipeak_chr'})
    all_peaks = all_peaks0.rename(columns={'chr': '#chr'})
...
if __name__=="__main__":
    ...
    parser.add_argument('--dist', default=1000,
                        help='Distance threshold between two paired peaks.')
