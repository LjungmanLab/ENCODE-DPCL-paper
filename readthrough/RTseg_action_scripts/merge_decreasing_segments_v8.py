...
def simple_merge(row, row_ind, df, prev_score, prev_chr):
    score = row.score
    _chr = row.chr
...
def merge_on_score(df):
    ...
    for row_ind, row_vals in tqdm(enumerate(df.iterrows()), total=len(df)):
        ...
        if (prev_score is not None) and simple_merge(row, row_ind, df, prev_score, prev_chr):  
    ...
    formatted_df = pd_df[['chr', 'start', 'end', 'ID', 'score', 'strand', 'feat_type', 'seg_length', 'fused_seg_ids', 'min_segment_score', 'num_fused_seg']]
...
def main(bed_filepath):
    ...
    if bed_filepath is None:
        bed_filepath = "~/Desktop/shared_biosamples/read-through/fused_dec/HCT1160h4001a.hg38.gencode_29.spikeins_1.bin_250.segments.actual_RPKM.bed"
