...
def check_6h_expression(df, df_exon_exp):
    ...
    Variables
    ---------
    6h gene expression cutoff can be modified (currently >0.5)
    '''
...
def main(working_file, sample, exon_exp_file):
    ...
    final_df = final_df.sort_values(by = ['RT_chr', 'RT_start'])
    eval_start = eval_start.sort_values(by = ['chr', 'start'])
    eval_end = eval_end.sort_values(by = ['chr', 'start'])
    eval_gene = eval_gene.sort_values(by = ['chr', 'start'])
